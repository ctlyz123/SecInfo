...
mongodb_mark = Mongodb().connect(dbsconf["host"], dbsconf["port"], "secinfo", "twitter_mark")


mongodb.con.create_index("hash", unique=True)
mongodb_trash.con.create_index("hash", unique=True)
mongodb_mark.con.create_index("hash", unique=True)
...
def delete():
    ...
    if request.form.get('mark'):
        mongodb_mark.delete({"hash":hash})
        return show()

    mongodb.update({"hash":hash},{"$set":{"flag":1}})
...
def show():
    ...
    if mark:
        a = mongodb_mark.select_all({"flag": 2}, {"_id": 0, "flag": 0})
    else:
...
def addmark():
    hash = request.form.get('hash')
    mongodb.update({"hash":hash},{"$set":{"flag":2}})
    delete_one = mongodb.select({"hash":hash},{"_id":0})
    try:
        mongodb_mark.insert(delete_one)
        mongodb.delete({"hash":hash})
    except:
        pass
    return show()
